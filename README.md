...
- Установите переменную окружения `VECTOR_DB` в ваш векторный магазин (например, `valkey`).
- Добавьте нужные параметры, например:
  ```env
  VECTOR_DB=valkey
  VALKEY_URL='valkey://localhost:6379'
  ```
- Перезапустите контейнер, чтобы изменения вступили в силу.

### 2. Доступ к UI RAG
- Откройте Open WebUI в браузере (по‑умолчанию `http://localhost:8083`).
- В левом меню выберите пункт **Knowledge** (или **RAG**).
- Нажмите **Create New Knowledge** для создания новой коллекции.

### 3. Загрузка документов
- Перетащите файлы в область загрузки или используйте кнопку **Upload**.
- Поддерживаемые форматы: `.txt`, `.pdf`, `.docx` и др. При загрузке автоматически будет выполнено встраивание (embedding) выбранной модели.
- Под именем каждого файла отображается **прогресс-бар**: первая половина — фактическая передача байтов на сервер, вторая — обработка и встраивание на стороне бэкенда (счётчик «Встраивание фрагментов N/M»). Прогресс продолжается даже в другой вкладке или после перезагрузки страницы.

### 4. Выполнение запроса с RAG
- Перейдите в чат.
- Для обращения к базе знаний используйте префикс `#`:
  ```
  #my_knowledge "Какова основная идея загруженного PDF?"
  ```
- Модель выполнит поиск релевантных фрагментов и включит их в ответ.

### 5. Визуализация GrafRAG
- После нескольких запросов откройте вкладку **Graph** внутри представления коллекции.
- Вы увидите граф, где узлы – документы/фрагменты, а рёбра – семантическая схожесть.
- Наведите курсор, чтобы увидеть отрывки текста.
- Используйте фильтры для настройки порога схожести и скрытия слабых связей.

### 6. Настройка параметров RAG
- В **Settings → RAG** можно изменить:
  - `EMBEDDING_MODEL` – используемая модель векторизации.
  - `RAG_TOP_K` – количество возвращаемых фрагментов.
  - `RERANK_MODEL` – модель для пере‑ранжирования.
- После изменения параметров рекомендуется перезапустить бэкенд.

### 7. Отладка
- Просмотрите логи контейнера (`docker logs open-webui`) на наличие ошибок соединения с векторным хранилищем.
- Убедитесь, что коллекция появилась в базе данных (например, запросом `SELECT * FROM knowledge;` при использовании PostgreSQL).
- С помощью API можно проверить список коллекций: `curl http://localhost:8083/api/v1/knowledge`.

### 8. Проверка работы
- После выполнения шагов, убедитесь, что:
  1. UI Knowledge загружается без ошибок.
  2. Загруженные файлы отображаются в списке.
  3. Запросы возвращают ответы с контекстом из знаний.
  4. На вкладке **Graph** отображается граф.

Если возникнут проблемы, обратитесь к разделу **Устранение неполадок** в этом README.
