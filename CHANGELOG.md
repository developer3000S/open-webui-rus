...
- 📄 **`.env` is the single source of deployment settings.** The file was previously decorative for a containerized run: `docker-run.sh` passed no environment at all, so the image's own `ENV` lines won and anything declared in `.env` (`VECTOR_DB`, `VALKEY_URL`, `NEO4J_*`, …) was ignored. `.env` is now mounted into the container, parsed by both `backend/start.sh` and `env.py` (with override), and the names it declares outrank the stored `config` table — which is what let a stale seeded row (e.g. `rag.embedding_batch_size = 32`) defeat a corrected default indefinitely. Settings that are not named in the file stay fully manageable from Admin → Settings, and saving there keeps working as before; for a name the file does declare, an admin edit now applies until the next restart instead of silently never being read back. `docker-compose.yaml` and `docker-compose.ollama.yaml` no longer duplicate `OLLAMA_BASE_URL` and friends in `environment:` blocks, which would otherwise silently beat the file.
- 🧬 **Duplicate documents reuse embeddings instead of recomputing them.** A file whose extracted text hashes to an already-indexed file's hash now gets those vectors copied rather than sent through the embedding server a second time — ~12.5 s per 460-token chunk on a CPU-only endpoint, in a corpus where the same document commonly appears several times. The saving is not limited to duplicates: the upload pipeline indexes a file into `file-{id}` and then into the knowledge base, and the second pass now borrows the vectors computed seconds earlier instead of paying for them again. Adoption requires the source's embedding engine and model to match, since vectors from another model live in an unrelated space and would corrupt retrieval silently; any mismatch (different model, different splitter, missing or empty source) falls back to normal embedding, so the setting never trades correctness for speed. Controlled by `RAG_DEDUP_DUPLICATE_FILES`, on by default.

### Fixed

- ⏳ **Uploads no longer freeze at "Uploading 0%".** A failed status stream, a failed upload, or a page refresh during an upload left the file row stuck at the very first progress tick, so the knowledge base showed a bar that never moved and no error was ever reported. The transfer-to-processing handover is now announced before the status stream is opened, a failed upload drops its row instead of leaving a frozen one, and the pending-files poll no longer overwrites this tab's live progress (or duplicates the row) while an upload is in flight.
- 🧊 **A wedged embedding server fails instead of hanging forever.** Embedding requests had no deadline at all (`AIOHTTP_CLIENT_TIMEOUT` defaults to waiting indefinitely), so a file could sit in `processing` with a stuck bar for hours and never surface an error. Embedding HTTP calls are now bounded, and the job is abandoned once it stops reporting progress, with the reason written to the file record so the UI can show it.
- 🚧 **Embedding requests no longer flood the embedding server.** Unbounded concurrency (the default) issued every batch of a document at once, which overloads Ollama into `503 server busy, maximum pending requests exceeded`, dropped connections, and exhausted file descriptors. A bounded default now applies when the setting is left at 0, and the default embedding batch size returns to 1 chunk per request — large batches left the bar visibly frozen on slow (CPU-only) endpoints. Set both explicitly in admin settings for a GPU-backed endpoint.
- 🚫 **Embedding cancellation actually stops processing.** Cancelling an embedding previously removed the file record while the background processing pipeline kept running and re-linked the file to the knowledge base, so it reappeared as completed. The pipeline now checks the file status at each checkpoint (before processing, before and after the embedding step, and before auto-linking), aborts cooperatively when a cancellation is detected, and cleans up any vectors already written for the cancelled file.
- 🧬 **The knowledge-base pass no longer re-splits chunks, so embedding reuse is actually taken.** An upload indexes a file into `file-{id}` and then into the knowledge base, and the second pass read those already-split chunks back only to chunk them a second time. Re-splitting normalizes whitespace and header boundaries, so no chunk came out with the bytes its vector was computed from and every reuse lookup was refused — measured 9 second passes out of 9 declined, one 280-chunk document re-embedded for 2 h 26 min despite its vectors being seconds old (the hash, engine and model all matched; only the text did not). Documents that arrive already chunked are now indexed as they are. Knowledge-base collections written before this fix keep their re-split chunks; re-add a file to rebuild them.
