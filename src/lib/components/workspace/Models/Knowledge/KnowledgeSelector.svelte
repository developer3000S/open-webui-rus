				...
				{#if browsingCollection}
					...
					{:else}
						...
						{#each browsingDirectories as dir (dir.id)}
							<div
								class="px-2.5 py-1 rounded-xl w-full text-left flex items-center text-sm hover:bg-gray-50 hover:dark:bg-gray-800 hover:dark:text-gray-100"
							>
								<button
									type="button"
									class="flex-1 flex items-center gap-1 text-black dark:text-gray-100"
								...
								>
									...
									<Tooltip
										...
										tippyOptions={{ zIndex: 100000 }}
									>
										<div class="line-clamp-1 flex-1 text-sm text-left">{dir.name}</div>
									</Tooltip>
								</button>
						...
						{#each browsingFiles as file (file.id)}
							<button
								class="px-2.5 py-1 rounded-xl w-full text-left flex items-center gap-1 text-sm hover:bg-gray-50 hover:dark:bg-gray-800 hover:dark:text-gray-100 text-black dark:text-gray-100"
								type="button"
								on:click={() => {
									dispatch('select', {
										...file,
							...
							>
								<Tooltip
									...
									tippyOptions={{ zIndex: 100000 }}
								>
									<DocumentPage className="size-4 flex-shrink-0" />
								</Tooltip>
								<Tooltip
				...
				{:else}
					...
					{#each items as item, i}
						...
						{#if item.type === 'collection'}
							<!-- Collection row: click to browse in, plus to select -->
							<div
								class="px-1.5 py-0.5 rounded-xl w-full text-left flex items-center text-sm hover:bg-gray-50 hover:dark:bg-gray-800 hover:dark:text-gray-100"
							>
								<button
									class="w-full flex-1"
									type="button"
						...
						{:else}
							<!-- Note or File row: existing behavior -->
							<div
